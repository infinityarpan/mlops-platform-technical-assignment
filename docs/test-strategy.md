...
API tests (`tests/api`) use SQLite and a fake enqueue that runs `process_deployment` in-process, so pytest does not need Redis. They cover the assignment scenarios: two versions, approve, block unapproved production, deploy, rollback, idempotency key, retry, viewer 403, 422.
